...
contains 525 events treated as complete. A simulator-gated partial pass keeps only `$0278` PARTIEL; its two user-validated Android-absent controller carriers are staged in `translations/dialogues_manual_supplements.json` and remain exact USA text until manually translated. The current corpus therefore contains
526 events / 1138 visible semantic source IDs / 1196 JSON entries. The remaining exclusions are 161 alignment-incomplete events and 17 formatter rejects; there are no remaining simulator rejects.
The existing choice-layout recovery keeps the conservative row-prefix, decoration and later-anchor repairs, and the updated simulator now also admits the five runtime-validated measured-end rows `$00CE/$00CF/$00D0/$00D1/$0202`. A Potos `$00CE` diagnostic showed that moving the **logical** first option anchor left to `$00` or `$01` clips its leading glyphs, so that parser/storage rewrite remains rejected. The accepted left-margin rule instead moves only component 06's private visual/highlight boundary while preserving `$A1D7[]`.

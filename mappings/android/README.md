...
contains 493 events treated as complete. A simulator-gated partial pass accepts 3 PARTIEL events while
suppressing 3 still-unresolved semantic IDs plus 2 mapped-but-layout-deferred IDs from visible dialogue, so PARTIEL scenes remain
French-only even when semantically incomplete. The current corpus therefore contains
496 events / 1051 visible French semantic source IDs / 1106 JSON entries. The remaining exclusions are 171 alignment-incomplete events, 17 formatter
rejects and 20 simulator rejects. Choice-anchor incompatibilities remain rejected rather

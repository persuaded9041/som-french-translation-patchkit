...
from shared.dialogue_codec import COMMAND_LENGTHS, COMMAND_NAMES
...
def decode_japanese_text(raw: bytes) -> str:
    """Decode a contiguous Japanese dialogue byte sequence to Unicode."""
    out: list[str] = []
    pos = 0
    while pos < len(raw):
        value = raw[pos]
        if value >= 0x80:
            out.append(DEFAULT_TABLE[value - 0x80])
            pos += 1
            continue
        if 0x60 <= value <= 0x67:
            ...
            raise ValueError(f"byte ${value:02X} is not Japanese dialogue text")
        end = pos + 1 + count
        if end > len(raw):
            raise ValueError(f"truncated Japanese S{shift} text run at +${pos:04X}")
        out.extend(_shift_lookup(shift, item) for item in raw[pos + 1 : end])
        pos = end
    return "".join(out)

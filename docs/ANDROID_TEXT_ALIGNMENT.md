...
**Current post-audit resolution:** `$035F` remains Android-identity-unmapped, but full runtime tracing established that its stock display is the composition `Dryad` + shared `$0360` (`'s magic will work!`). Since the French formatter now neutralizes `$0360` for the seven Android-aligned elemental branches, the user validated restoring the full manual `$035F` payload `Dryade fera réagir l'orbe !`; `$0360` stays neutralized. This supersedes the Round-60 *active payload* decision without changing the historical JP-carrier fact (`ドリアード`) or Android identity.

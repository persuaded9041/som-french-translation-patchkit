...
- `translations/dialogues_french.json`: generated by the conservative
  simulator-filtered Android dialogue pass. The current file contains 392 complete
  events / 675 translated semantic source tokens (692 JSON entries including
  ...
  have representative runtime validation; the complete 392-event corpus remains a
...
`--only dialogue-format-mass`: **392 complete events / 675 translated semantic
source tokens** (692 JSON entries including generated layout controls). The mass pass still refuses mixed-language events: every semantic

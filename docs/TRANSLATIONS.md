...
- `translations/dialogues_french.json`: generated by the conservative
  simulator-filtered Android dialogue pass. The current Round-67 corpus contains
  **695 simulator-clean events / 1687 accepted semantic source IDs / 1783 JSON entries**:
  **669 complete + 26 PARTIEL**. Semantic Android identity remains **1798/1838 (97.8%)**;
  manual supplements and explicit SNES-JP-based suppressions do not inflate that counter.
  The authoritative PARTIEL/validation state is in `docs/HANDOFF.md`. In PARTIEL events,
  proven mappings may be emitted in French while unresolved carriers remain stock English,
  reviewed Android-absent SNES content may be staged in
  `translations/dialogues_manual_supplements.json`, and only exact user-approved
  structural omissions may be suppressed. Round 58 changed the supplement file to a
  ...
  outside Android identity. Choice rows use component 06's ordinary
...
`mappings/android/dialogues_manual_supplements_round67.html`; regenerate/check it with
...
expanded payload `Dryade fera réagir l'orbe !` remains withdrawn. The full mass pass is **695 events with
...
reproducible with `--only dialogue-format-page-pilot`. The current `translations/dialogues_french.json`
is generated by `--only dialogue-format-mass`: **695 simulator-clean events / 1687 accepted
semantic source IDs / 1783 JSON entries**, comprising **669 complete + 26 PARTIEL**.
Exclusions are **6 alignment-incomplete + 2 formatter-rejected + 1 simulator-rejected**.
The authoritative PARTIEL list is in `docs/HANDOFF.md`. PARTIEL events are dynamically
re-evaluated on every mass pass. `$0278` is special because accepted Android text is preserved,

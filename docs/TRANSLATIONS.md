...
- `translations/dialogues_french.json`: generated by the conservative
  ...
  496 simulator-clean events / 1051 visible French semantic source IDs (1106 JSON entries
  including generated layout/suppression entries). Of these, 3 are PARTIEL events that
  suppress 3 still-unresolved semantic IDs plus 2 mapped-but-layout-deferred IDs so the visible event does not mix French and English. `$01DC` is instead user-validated as a complete Android adaptation and omits its final stock `PLAYER_NAME(0)` together with Android-absent `C9:804A`. The `$0107` baseline, `$010F` sentence-aware
  extra-page transition, semantic line-placement rules and interactive-WAIT cleanup
  have representative runtime validation; component 06's stock-rendered choice fallback
  is runtime-validated on `$0331`. The full 496-event corpus still requires playthrough.
...
`--only dialogue-format-mass`: **496 simulator-clean events / 1051 visible French semantic
source tokens** (1106 JSON entries including generated layout/suppression entries). The

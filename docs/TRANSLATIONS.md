...
- `translations/dialogues_french.json`: generated by the conservative
  ...
  404 complete events / 719 translated semantic source tokens (737 JSON entries including
  generated layout controls). The `$0107` baseline, `$010F` sentence-aware
  ...
  is runtime-validated on `$0331`. The full 404-event corpus still requires playthrough.
...
`--only dialogue-format-mass`: **404 complete events / 719 translated semantic
source tokens** (737 JSON entries including generated layout controls). The mass pass still refuses mixed-language events: every semantic
source ID in an accepted event must already be confidently aligned. It may use

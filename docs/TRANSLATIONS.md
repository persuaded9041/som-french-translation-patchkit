...
- `translations/dialogues_french.json`: generated by the conservative
  ...
  500 simulator-clean events / 1066 visible French semantic source IDs (1119 JSON entries
  including generated layout/suppression entries). Of these, 2 are PARTIEL events that
  suppress 3 still-unresolved semantic IDs so the visible event does not mix French and English. `$01DC` is instead user-validated as a complete Android adaptation and omits its final stock `PLAYER_NAME(0)` together with Android-absent `C9:804A`. The `$0107` baseline, `$010F` sentence-aware
  ...
  The full 500-event corpus still requires playthrough.
...
`--only dialogue-format-mass`: **500 simulator-clean events / 1066 visible French semantic
source tokens** (1119 JSON entries including generated layout/suppression entries). The
established complete-event pass is followed by a conservative partial-event pass: only
already accepted mappings are translated, while every unresolved semantic ID is encoded
as empty text so the visible event does not fall back to stock English. Partial events
do not receive compact/event-level repair and are admitted only if their direct
French-only-but-incomplete serialization simulates cleanly. The formatter may use

...
- `translations/dialogues_french.json`: generated by the conservative
  simulator-filtered Android dialogue pass. The current file contains 331 complete
  events / 454 translated source tokens. The `$0107` baseline, `$010F` sentence-aware
  extra-page transition, semantic line-placement rules and interactive-WAIT cleanup
  have representative runtime validation; the complete 331-event corpus remains a
  full-playthrough candidate.
...
`--only dialogue-format-mass`: **331 complete events / 454 translated source
tokens**. The mass pass still refuses mixed-language events: every semantic
source ID in an accepted event must already be confidently aligned. It may use
the full three-line physical page and at most one generated extra page per
mapping, then independently serializes/simulates each event. Any simulator error,
...
scroll lines immediately after an existing WAIT into a clear-only `TEXT_CLEAR`. The

...
6. `06_dialogue_vwf` - runtime-validated variable-width renderer for stock `$C9/$CA` event dialogue and component-08 relocated `$E8-$EC` events under the same caller gate; interactive choice rows use the same VWF path; stock selection/highlight geometry, terminal-boundary handling, and the minimal later-anchor shift needed by long localized labels are runtime-validated on the Potos test path.
...
**simulator-filtered mass pass**. The current corpus contains **510 simulator-clean events /
1078 visible semantic source IDs / 1129 JSON entries**: **509 complete + 1 PARTIEL**.
`$0278` deliberately remains PARTIEL because its START/L/R controller instructions are
SNES-only; those two user-validated Android-absent carriers are staged in
`translations/dialogues_manual_supplements.json` and currently retain their exact USA text
until manually translated. The common inn flow is complete through a reviewed parameterized
Android template (ID 110): the stock numeric price remains dynamic while the surrounding
French comes from Android. Semantic alignment itself remains **1601 / 1838 (87.1%)**, with
237 unresolved; manual supplements and parameterized rendering do not inflate that count.
The independent simulator reports **0 errors, 0 warnings and 0 implicit runtime wraps**.
The remaining exclusions are 161 alignment-incomplete events, 17 formatter rejects and
16 simulator rejects.

...
- `french_resources` - deterministic reinsertion of reviewed French `$CA` resources: established name families plus the nine top-level Ring Menu labels, and the two reviewed shop currency literals (`GP -> PO`). Android-derived text uses the fingerprint-validated generated cache; SNES-specific reviewed wording/literals come from dedicated JSON overrides, never hard-coded in the builder.
- `french_shop_text` - French `$D9` shop/forge response mini-events. Six lines are verified direct Android-FR payloads; three user-validated SNES adaptations live in a dedicated override JSON. The text component keeps the stock parser and enforces the validated 28-character limit; when `vwf_ui` is present, those same rows are rendered through the dedicated Shop VWF tag. The compacted pool stays inside `$D9:FE20-$FEF3`.
...
After the 2026-09-16 shop merchandise VWF / `PO` / standalone-dependency promotion, the current `patches/all.ips` SHA-256 is `f4e37892d3946b54493c0c93c2b9d35571dfd51670970e31f0925aaccd52e61e`; `vwf_ui.ips` is `37b840462d39e9653f1c11fe85d5a6db97df17daa75a13c636bff6c88da0c896`, `french_resources.ips` is `718af6e165e7892c7be69ec809195fc864dd41e2ebbb847dca7949a09d98c8b2`, and `french_shop_text.ips` remains `b4530dfb6f9c25e7b28229448ef90f435858a351d27de843ea54507cd04b8271`.

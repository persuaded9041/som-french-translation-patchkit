...
**701 simulator-clean playable events / 1815 accepted semantic source IDs / 1947 active sparse
...
For the current development checkpoint and next work, see `docs/HANDOFF.md`. The next maintenance session is scoped in `docs/MAINTENANCE_NEXT.md`: profile/accelerate `dialogue-format-mass` and reduce historical repository clutter without changing generated semantics. The accepted non-dialogue UI-VWF architecture and extension rules are summarized in `docs/UI_VWF.md`. The rejected Watts forge experiments and the runtime proof chain remain in `docs/FORGE_VWF_RESEARCH.md`; read both files before extending `vwf_ui`.

...
- `vwf_ui` - standalone VWF extensions for proven non-dialogue UI paths: Watts' Forge row keeps its exact +3/tagged suffix-compaction backend, while the top-level Ring Menu title row has its own one-shot tag and exact +4 stock-buffer margin for 32-character French labels.
- `french_resources` - deterministic reinsertion of reviewed French `$CA` resources: established name families plus the nine top-level Ring Menu labels. Android-derived text uses the fingerprint-validated generated cache; SNES-specific reviewed wording is loaded from `translations/text_resources_reviewed_overrides.json`, not hard-coded in the builder.

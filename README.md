...
**simulator-filtered mass pass**. The current corpus contains **392 complete
events / 675 translated semantic source IDs / 692 JSON entries**. Of 704 semantic
text events, 431 are completely aligned, 415 pass the formatter, and 392 pass the
independent simulator with **0 errors, 0 warnings and 0 implicit runtime wraps**.
The remaining exclusions are 273 incomplete alignments, 16 formatter rejects and
23 simulator rejects; every simulator reject currently contains unsupported
interactive choice layout. Unmapped or excluded text remains stock English.
...
rules. Unsupported structures are rejected rather than guessed, and the complete
392-event corpus remains a full-game playthrough candidate.

...
- `french_font` - validated French global fixed-font atlas, used by the French aggregate and kept out of the USA-only aggregate.
...
- `french_vwf_intro` - French metrics/encoding overlay applied after `default_vwf_intro`.
...
- `french_vwf_dialogues` - French DTE, glyph and metric overlay applied after `default_vwf_dialogues`.
...
- `french_vwf_ui` - French glyph and metric overlay applied after `default_vwf_ui`.

...
**simulator-filtered mass pass**. The current Round-67 checkpoint contains **695 simulator-clean events /
1687 accepted semantic source IDs / 1783 JSON entries**: **669 complete + 26 PARTIEL**.
Semantic Android alignment remains **1798 / 1838 (97.8%)**, with **40 unresolved semantic IDs**;
the remaining exclusions are **6 alignment-incomplete + 2 formatter-rejected + 1
simulator-rejected event**. The independent simulator reports **0 errors, 0 warnings and
...
The current development state is **Round 67**. Dialogue identity remains **1798 / 1838 (97.8%)**, while the simulator-clean payload contains **695 events = 669 complete + 26 PARTIEL**, **1687 accepted semantic source IDs / 1783 JSON entries**, with **6 alignment-incomplete + 2 formatter-rejected + 1 simulator-rejected** exclusions. Simulation is **0 errors / 0 warnings / 0 implicit wraps**.

Current user-facing review material:

- `docs/HANDOFF.md` — authoritative current state, locked decisions and next work;
- `mappings/android/dialogues_review_worklist_round67_simplified.html` — remaining non-manual review queue (**15 events / 55 carriers**);
- `mappings/android/dialogue_04E2_android_fr_round67.html` — complete Android EN/FR context for the five deferred `$04E2` carriers;
- `mappings/android/dialogues_manual_supplements_round67.html` — manual JP/USA/official-FR provenance and approval state.

Round-by-round Android identity and formatter research is preserved in `docs/ANDROID_TEXT_ALIGNMENT.md` and `docs/DIALOGUE_FORMAT.md`. Do not use old round counters from those historical sections as the current checkpoint.
...
otherwise be overwritten. The 695-event corpus as a whole still requires the planned

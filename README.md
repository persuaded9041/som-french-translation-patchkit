...
6. `06_dialogue_vwf` - runtime-validated variable-width renderer for stock `$C9/$CA` event dialogue and component-08 relocated `$E8-$EC` events under the same caller gate; proven `CHOICE_BEGIN` rows are routed through the complete stock fixed-width renderer so selection highlighting stays aligned.
...
**simulator-filtered mass pass**. The current partial-aware corpus contains **496
simulator-clean events / 1051 visible French semantic source IDs / 1106 JSON entries**.
The corpus now contains 493 events treated as complete plus 3 explicitly PARTIEL events. Three of the 493 (`$0103`, `$017F`, `$01DC`) are user-validated Android adaptations whose remaining SNES-only fragments stay unmapped and suppressed without a PARTIEL badge. The partial path suppresses **3 still-unresolved semantic source IDs plus 2 mapped-but-layout-deferred IDs** from visible dialogue. Structural commands/layout remain stock except for the user-validated `$01DC` omission, which drops the final stock `PLAYER_NAME(0)` together with its Android-absent `C9:804A` reply. The independent
simulator reports **0 errors, 0 warnings and 0 implicit runtime wraps** across all 496
events. Partial events are intentionally French-only but may be semantically incomplete. The remaining exclusions are 171
alignment-incomplete events, 17 formatter rejects and 20 simulator rejects. Choice-anchor
incompatibilities remain rejected rather than guessed; `$0202` remains separately rejected
for visible-bitmap overflow.
...
`PLAYER_NAME`, fresh-line `TEXT_X` and existing event interruptions are handled
...
structures are rejected rather than guessed. The stock-rendered choice-row
fallback is runtime-validated on event `$0331`: the complete `(Yes  No)` row and stock
selection highlighting remain aligned. The 496-event corpus as a whole still requires
the planned full-game playthrough.

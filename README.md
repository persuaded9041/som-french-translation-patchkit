...
components 02-05 have been migrated there. Component 08 consumes
`translations/dialogues_french.json`, currently generated by the **simulator-filtered
mass pass**: 331 complete events / 454 translated semantic source IDs (458 JSON
entries including generated layout-only controls). Of 704 semantic text events, 431
are completely aligned, 366 are formatter-compatible, and 331 pass the independent
simulator with zero error, warning or implicit runtime wrap.

The formatter's core runtime-validated checkpoints include the `$0107` dual
240-pixel/38-unit layout and `$010F` sentence-aware 3+1 pagination with generated
`WAIT $00` + `TEXT_CLEAR`. Representative runtime tests also validate semantic speaker
turns, punctuation attachment, sentence-aware line balancing, clean WAIT-separated
pages and the exact interactive-`WAIT $00` rolling-window overlap repair. Timed
`WAIT $04/$08` pauses are never rewritten by that cleanup. The complete 331-event
expansion as a whole remains a full-game playthrough candidate. Excluded events stay
entirely stock/English and are listed in
`mappings/android/dialogues_format_mass_excluded.csv`.

The user-validated static guardrail is `tools/simulate_dialogues.py`; it
re-decodes final serialized event bytes and is documented in `docs/DIALOGUE_SIMULATOR.md`.

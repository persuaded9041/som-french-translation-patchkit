...
**simulator-filtered mass pass**. The current choice-aware corpus contains **404 complete
events / 719 translated semantic source IDs / 737 JSON entries**. Of 704 semantic
text events, 431 are completely aligned, 417 pass the formatter, and 404 pass the
independent simulator with **0 errors, 0 warnings and 0 implicit runtime wraps**.
The remaining exclusions are 273 incomplete alignments, 14 formatter rejects and
13 simulator rejects; every simulator reject is a choice whose translated text
would overlap a stock option anchor or exceed the fixed selectable row. Unmapped or excluded text remains stock English.
...
`PLAYER_NAME`, fresh-line `TEXT_X`, existing event interruptions and exact
interactive-WAIT overlap repairs are handled only by explicitly proven structural
rules. Unsupported structures are rejected rather than guessed. The stock-rendered choice-row
fallback is runtime-validated on event `$0331`: the complete `(Yes  No)` row and stock
selection highlighting remain aligned. The 404-event corpus as a whole still requires

...
; The intro is identified exactly like `vwf_intro`: bank $CA and live event
...
;
; These bytes are unused by `vwf_intro`'s $CA intro renderer. `vwf_dialogues`
; also uses them for ordinary event dialogue, including $CA, but `vwf_intro`
; intercepts translated event $0400 before `vwf_dialogues` reaches its entry hook.

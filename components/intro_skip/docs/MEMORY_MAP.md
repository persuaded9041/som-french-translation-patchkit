...
`$7E:9380-$9389` and whose parser buffer is `$7E:9390-$93BB`. `vwf_dialogues` also uses `$7E:938A-$938B` for its width-table index, including
ordinary `$CA` event dialogue. During translated intro event `$0400`, component
05 intercepts the renderer before `vwf_dialogues` reaches its entry hook, keeping

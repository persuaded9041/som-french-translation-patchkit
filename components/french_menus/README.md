...
- Status / Caractéristiques money total now prefixes its separately rendered currency unit with the otherwise-free byte `$C7:7B69`, and redirects the local unit pointer `$CE:E9BC` from `$7B6A` to `$7B69`. This now yields `1234567 PO` both standalone and in the aggregate, without VWF or relocation.
...
unchanged. Runtime validation was confirmed by the user on 2026-09-26; the
corresponding `french_menus.ips` / `all-fr.ips` baseline is promoted. Full
provenance and static checks remain documented in
`docs/SKILL_MENU_HELP_RESEARCH.md`; the candidate directory is historical.

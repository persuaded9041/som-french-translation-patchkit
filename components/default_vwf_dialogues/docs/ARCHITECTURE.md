...
This document is the authoritative technical description of `vwf_dialogues`'s
...
At `$C0:167D`, before the stock renderer has pushed local values, `vwf_dialogues`
reads the caller's 16-bit return address at `1,S`. It tags the invocation as VWF
only when the stacked return is `$1152` and the live event bank is stock `$C9/$CA` or the `french_dialogues` relocation range `$E8-$EC`.
A private flag at `$7E:9385` carries that decision through the internal renderer
hooks. Every invocation reaching `vwf_dialogues` clears the flag before making the
decision, so non-event calls always fall back to stock behavior.

Both sides of this gate are runtime-validated: `$CA` story dialogue uses the VWF while GAME SELECT remains fixed-width, and `french_dialogues` event `$0107` has been executed successfully after relocation to `$E8:2000`.
...
`$235B`). `vwf_intro` and `vwf_dialogues` now install one byte-identical private-buffer
bridge. `vwf_dialogues` enables dialogue mode only for `$114B` plus live bank
...
`vwf_dialogues` still keeps `$C0:168A-$C0:16B0` intact. The per-character hook now
...
`vwf_dialogues` changes placement/composition of the already selected glyph row.

## Shared row compositor

The runtime-validated shared-compositor refactor moves only the renderer-neutral 8x12 row
composition primitive into `shared/vwf/compositor.py`. Both components install
...
lives in `shared/vwf/metrics.py`, and the common runtime selector lives at
...
compositor. `vwf_intro` calls it directly from its 12-row loop. `vwf_dialogues`'s
...
`shared/vwf/framing.py` at `$C7:44C0-$4557`. `vwf_dialogues` reaches it through
...
`vwf_dialogues` therefore replaces only the source fetch at `$C0:16EA-$16ED` with
...
`vwf_dialogues` therefore preserves two runtime paths without introducing a second choice renderer:
...
**Undecorated two-option choices.** `$A1D7[]` remains untouched and continues to define parser/storage resets, but rendering records the actual VWF cursor after each non-space glyph. At a logical boundary the visual helper derives separate cell-aligned coordinates: first option at `max(logical_first, $03) - 2`; second option normally at `ceil(previous_real_end / 8) + 1`; if the rounded first endpoint is already cell `$11` or later, the extra blank cell is omitted and the second option starts at the rounded endpoint; terminal at `ceil(final_real_end / 8)`. The resulting private boundaries live at `$93BD-$93BF` and become valid only after the terminal boundary is reached.
...
This measured-end separation is runtime-validated on Potos reproductions of `$00CE`, `$00CF`, `$00D0`, `$00D1` and `$0202`, with both selected options checked. In the final two-cell-left geometry, `$00D0` starts at 8 px, reaches 117 px, starts the second option at 128 px after the normal separator, finishes at 209 px and rounds its terminal highlight boundary to cell `$1B`. The cell-`$11` separator-omission branch remains in the helper as a generic right-edge guard; it was independently runtime-validated on the earlier `$00D0` stress checkpoint before the two-cell private left compaction reduced that row enough to stop triggering it. The rule is structural and measured; it does not inspect event IDs, text, or `$A1D7[]` contents beyond the ordinary logical boundaries.

The rejected first-anchor-left diagnostics (`$00/$0F`, `$01/$10` on `$00CE`) remain rejected because they clipped the first label. The validated rule is visual-only: keep logical storage untouched, clamp the logical first anchor to at least `$03`, then shift the private first visual/highlight boundary two cells left.

### Potos `$05/$0A` diagnostic trap

Event `$0331` is a convenient runtime test carrier, but its stock `Oui / Non` row uses logical `CHOICE_OPTION` anchors `$05/$0A`. Those bytes are not cosmetic spacing: `$5A xx` resets the decoded-buffer write position to `xx`, and stock also records the same values in `$A1D7[]`. Replacing the short labels with a long wide-choice pair while leaving `$05/$0A` intact therefore makes the second `$5A $0A` rewind into the first localized label; the visible symptom is a truncated first option such as `Déser`. Removing the parentheses does not change that storage reset.

A wide-choice diagnostic must reproduce the **logical storage geometry** required by the translated labels before judging `vwf_dialogues`'s private visual geometry. For the `$00D0` reproduction, the first anchor is `$03` and `Désert de Kakkara` occupies 17 decoded slots, so the minimum non-overwriting second anchor is `$03 + $11 = $14`. The validated diagnostic therefore used `$03/$14` with the outer decoration stripped. `vwf_dialogues` still never rewrites `$A1D7[]`: `$03/$14` protects parser storage, while measured-end privately places the second option much earlier on screen. This is test-harness discipline, not an event-specific runtime rule.

## Outline repair

The stock outline routine `JSR $162C` remains untouched. `vwf_dialogues` hooks after
...
requires the exact `vwf_dialogues` renderer-active tag `$7E:9385 == $01`, which is
structurally narrower than a raw bank test: ordinary `vwf_dialogues` dialogue sets
exactly `$01`, inactive renderer calls leave `$00`, and `vwf_intro`'s
mutually-exclusive intro renderer uses the same byte only for validated glyph
advances `3..8`. `vwf_intro`'s builder asserts that none of its emittable glyphs
...
- keep the post-outline repair gated by exact `vwf_dialogues` tag `$9385 == $01`; do not replace it with a bank-only test.

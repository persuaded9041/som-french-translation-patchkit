...
`vwf_dialogues` is runtime-validated for stock `$C9/$CA` event dialogue and for
`french_dialogues` relocated event banks `$E8-$EC` under the same caller gate. The
forced `$0107` relocation probe has been removed; `french_dialogues` now relocates

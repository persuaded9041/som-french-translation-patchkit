...
`vwf_dialogues` composes glyphs at a cumulative pixel cursor, so the two quantities
...
`vwf_intro` uses the same architectural principle for its intro-specific `$28`

...
from shared.vwf.renderer_runtime import (  # noqa: E402
    CHAR_START_FILE,
    FONT_ROW_FILE,
    CHAR_END_FILE,
    OUTLINE_POST_FILE,
    CHAR_START_HELPER_FILE,
    CHAR_END_HELPER_FILE,
    FONT_ROW_HELPER_FILE,
    WIDTH_TABLE_FILE,
    OUTLINE_POST_HELPER_FILE,
...
def make_parser_fetch_helper() -> bytes:
    ...
    # The hook is global but `vwf_dialogues` owns only shared parser mode 2.
    ...
    # Event/control token. Choice commands intentionally receive no special
    # parser treatment; choice rows use the ordinary dialogue VWF path.
    a.label("control")
...
def make_entry_helper() -> bytes:
    ...
    Tag only $1152 and then accept stock event banks $C9/$CA plus `french_dialogues`
    relocated banks $E8-$EC. `vwf_intro` intercepts
    its translated intro before this point, so its private $CA renderer remains
    isolated. $9385 is shared with `vwf_intro` only under mutually exclusive
    ...
    # Every accepted event-render invocation, including CHOICE_BEGIN rows,
    # enters the ordinary VWF path. No choice-only renderer state is kept.
...
# Framing selector source is shared with `vwf_intro`.  `vwf_dialogues` keeps
...
def main() -> None:
    ...
    print("Caller-gated dialogue VWF; choice rows use the ordinary VWF path")

...
;
; $7E:9385 is the per-invocation `vwf_dialogues` active tag. It overlaps the intro
; renderer's glyph-advance scratch only because the two renderers are mutually
; exclusive: `vwf_intro` exits before $C0:167D for event $0400.
;
; Runtime-validated post-outline scope: the repair at $C0:1168 gates on exact
; `vwf_dialogues` active tag $9385 == $01, covering ordinary C9/CA dialogue and relocated E8-EC dialogue while
; excluding `vwf_intro` intro, where $9385 is a glyph advance in the validated
; 3..8 range.

hirom

; Shared parser hooks/helpers are emitted byte-identically by `vwf_intro`
; and 06 from ../../shared/vwf_text_buffer.py. `vwf_dialogues` writes marker $06
...
dialogue_char_start:
    ...
    ; Y=floor(pixel_cursor/8)*12 for active `vwf_dialogues` dialogue. When the
...
dialogue_outline_post:
    ...
    ; exact `vwf_dialogues` active tag $9385 == $01, then repairs cross-cell outline
    ; boundaries for ordinary tagged C9/CA or relocated E8-EC dialogue before replaying stock tail.
    ; `vwf_intro` intro uses $9385 only for advances 3..8 and therefore skips.

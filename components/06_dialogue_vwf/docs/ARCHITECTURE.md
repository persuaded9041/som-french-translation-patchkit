...
A wide-choice diagnostic must reproduce the **logical storage geometry** required by the translated labels before judging component 06's private visual geometry. For the `$00D0` reproduction, the first anchor is `$03` and `Désert de Kakkara` occupies 17 decoded slots, so the minimum non-overwriting second anchor is `$03 + $11 = $14`. The validated diagnostic therefore used `$03/$14` with the outer decoration stripped. Component 06 still never rewrites `$A1D7[]`: `$03/$14` protects parser storage, while measured-end privately places the second option much earlier on screen. This is test-harness discipline, not an event-specific runtime rule.

...
- The main GAME FILE/save-menu resource is relocated from `C7:7340` to `C7:4D40`. This allows `FILE_LABEL` to grow from the stock `FILE` to `Fichier` while preserving the native resource structure. The small FILE frame descriptor at `C7:7585` is widened from `$03` (6 text cells) to the runtime-validated `$04` (8 text cells), leaving room for the full label without extending into the following dynamic slot text.

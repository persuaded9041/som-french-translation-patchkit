...
def make_vwf_font(rom: bytearray) -> tuple[bytes, bytes]:
    ...
    `vwf_intro` now reads the stock font directly and calls the shared runtime
    ...
    for glyph_index in range(128):
        ...
        if code in INTRO_RENDER_CODES:
            ...
            # policy as `vwf_dialogues`. Keep legacy data for unreachable table
...
def main(source_rom: Path, output_path: Path, patched_rom: Path | None = None) -> None:
    ...
    # `vwf_dialogues` uses exact value $01 as its persistent renderer-active tag.

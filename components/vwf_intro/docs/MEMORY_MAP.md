...
| `0x074C80-0x074C82` | `$C7:4C80-$4C82` | `vwf_intro` intro private-buffer marker + exclusive end pointer |
| `0x074C90-0x074CCE` | `$C7:4C90-$4CCE` | shared 63-byte 8×12 row compositor, installed byte-identically by `vwf_intro` / `vwf_dialogues` |
...
The relocated stock-event block ends before `intro_skip` at `$CA:FFC0`.
...
`$7E:9390-$93BB` is now the shared decoded-text private buffer for `vwf_intro`
and 06. Parser mode `$7E:9380` is 1 for the translated intro and 2 for `vwf_dialogues`
dialogue decoding; `vwf_intro` reuses `$9380` as its character count only after
parsing has finished. Renderer scratch remains mutually exclusive because component
05 intercepts event `$0400` before `vwf_dialogues` reaches `$C0:167D`.
